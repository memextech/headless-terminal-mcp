...
async function handleSendKeys(args: z.infer<typeof SendKeysArgs>) {
...
async function handleTakeSnapshot(args: z.infer<typeof TakeSnapshotArgs>) {
...
async function handleExecuteCommand(args: z.infer<typeof ExecuteCommandArgs>) {
...
async function handleListSessions() {
    ...
    const sessionsList = result.data.sessions.map((session: any) => 
        `- ${session.id} (${session.isAlive ? 'alive' : 'dead'}) - Created: ${session.createdAt}`
    ).join('\n');
...
async function handleCloseSession(args: z.infer<typeof CloseSessionArgs>) {
...
export function registerTools(server: McpServer) {
    ...
    server.tool(
        'ht_send_keys',
        'Send keys to an HT session. Keys can include text and special keys like "Enter", "Down", "Up", "^c" (Ctrl+C), etc.',
        SendKeysArgs.shape,
        handleSendKeys
    );
    ...
    server.tool(
        ...
        TakeSnapshotArgs.shape,
        handleTakeSnapshot
    );

    server.tool(
        'ht_execute_command',
        'Execute a command in the terminal and return the output. This combines sending the command + Enter key + taking a snapshot.',
        ExecuteCommandArgs.shape,
